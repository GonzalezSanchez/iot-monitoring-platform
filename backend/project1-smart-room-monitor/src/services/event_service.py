    ...
    def process_event(self, event_data: dict) -> Dict:
        ...
        try:
            ...
            if not self._update_room_state(event):
                logger.warning(f"Failed to update room state for {event.room_id}, continuing anyway")
                # Don't fail entire process if room update fails
            
            logger.info(f"Successfully processed event {event.event_id} for room {event.room_id}")
    ...
    def _update_room_state(self, event: SensorEvent) -> bool:
        ...
        try:
            ...
            if event.status in ['warning', 'alert']:
                ...
                logger.warning(f"Alert detected for room {room.room_id}: {event.status}")
